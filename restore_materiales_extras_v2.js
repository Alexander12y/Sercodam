...
function parseRegistrosFile(filePath) {
  try {
    ...
    for (let i = 0; i < lines.length; i++) {
      if (lines[i].includes('id_item') && lines[i].includes('id_material_extra')) {
        headerIndex = i;
        break;
      }
    }
    ...
    if (headerIndex === -1) {
      throw new Error('No se encontró la línea de encabezados con "id_item" y "id_material_extra"');
    }
    
    console.log(`Encabezados encontrados en línea ${headerIndex}`);
    
    // Obtener los registros después de los encabezados y separadores
